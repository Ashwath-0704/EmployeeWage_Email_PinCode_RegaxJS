...
function calculateTotalWage(Array, arr, arrNew) {
    ...
    let dailyWage = Array.map(function(element) {
        if ((element != -Infinity) == true) return 1 * RATE * Fixed_hrs;
    });
    let dailywagePartTime = arr.map(function(element) {
        if ((element != -Infinity) == true) return 1 * RATE * Part_Time_Fixed_hrs;
    });
    let dailywagelose = arrNew.map(function(element) {
        if ((element != -Infinity) == true) return 1 * RATE * Fixed_hrs;
    });
